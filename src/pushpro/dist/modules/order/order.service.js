...
let OrderService = OrderService_1 = class OrderService {
    constructor(orderEntity, cramiPackageEntity, userEntity, payService, globalConfigService, invitationService) {
    ...
    async handlePaymentSuccess(orderId) {
        try {
            ...
            if (!order || order.status !== 1) {
                this.logger.log(`订单 ${orderId} 不存在或未支付成功，跳过邀请返佣处理`);
                return;
            }
            await this.invitationService.processRecharge(order.userId, order.total);
            this.logger.log(`处理订单 ${orderId} 的邀请返佣成功`);
        }
        catch (error) {
            this.logger.error(`处理订单 ${orderId} 的邀请返佣失败: ${error.message}`, error.stack);
        }
    }
};
