...
let ChatService = class ChatService {
    ...
    async chatProcess(body, req, res) {
        ...
        const functionId = useModel || `${modelType === 1 ? 'chat' : 'draw'}_${useModel}`;
        await this.userBalanceService.validateBalance(req, deductType, deduct, functionId);
        ...
        try {
            if (res) {
                ...
                try {
                    if ((useModel === 'dall-e-3' ||
                        ...
                        if (response.status !== 5) {
                            ...
                            await this.userBalanceService.deductFromBalance(req.user.id, deductType, charge, 0, functionId);
                        }
                        else {
                        ...
                        await this.userBalanceService.deductFromBalance(req.user.id, deductType, charge, promptTokens + completionTokens, functionId);
                ...
                await this.userBalanceService.deductFromBalance(req.user.id, deductType, charge, functionId);
    ...
    async ttsProcess(body, req, res) {
        ...
        try {
            ...
            await Promise.all([
                ...
                this.userBalanceService.deductFromBalance(req.user.id, deductType, deduct, 0, functionId),
            ]);
