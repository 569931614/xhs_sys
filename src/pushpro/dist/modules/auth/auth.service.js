...
let AuthService = class AuthService {
    ...
    async register(body, req) {
        const { password, contact, code, inviteCode } = body;
        let email = '', phone = '';
        const isEmail = /\S+@\S+\.\S+/.test(contact);
        const isPhone = /^\d{10,}$/.test(contact);
        common_1.Logger.debug(`Contact: ${contact}, isEmail: ${isEmail}, isPhone: ${isPhone}, inviteCode: ${inviteCode || '无'}`);
        ...
        if (inviteCode) {
            newUser.invitedBy = inviteCode;
            common_1.Logger.debug(`用户注册时使用了邀请码: ${inviteCode}`);
        }
        ...
        if (inviteCode) {
            try {
                const inviter = await this.userService.findUserByInviteCode(inviteCode);
                if (inviter) {
                    common_1.Logger.debug(`发现邀请人，ID: ${inviter.id}，用户名: ${inviter.username}`);
                    const invitation = {
                        inviterId: inviter.id,
                        inviteeId: u.id
                    };
                    await this.userService.createInvitation(invitation);
                    common_1.Logger.debug(`创建邀请记录成功: 邀请人ID ${inviter.id} -> 被邀请人ID ${u.id}`);
                    await this.userService.incrementInviteLinkCount(inviter.id);
                    common_1.Logger.debug(`更新邀请人(${inviter.id})的邀请计数`);
                }
                else {
                    common_1.Logger.warn(`未找到使用邀请码 ${inviteCode} 的邀请人`);
                }
            }
            catch (error) {
                common_1.Logger.error(`处理邀请关系时出错: ${error.message}`, error.stack);
            }
        }
