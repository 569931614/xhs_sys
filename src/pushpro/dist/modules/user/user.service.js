...
let UserService = UserService_1 = class UserService {
    constructor(userEntity, invitationRepository, connection, verificationService, mailerService, userBalanceService, globalConfigService, configEntity) {
    ...
    async findUserByInviteCode(inviteCode) {
        this.logger.debug(`尝试根据邀请码 ${inviteCode} 查找用户`);
        try {
            const user = await this.userEntity.findOne({
                where: { inviteCode }
            });
            if (user) {
                this.logger.debug(`找到邀请码 ${inviteCode} 对应的用户：ID ${user.id}`);
            }
            else {
                this.logger.debug(`未找到邀请码 ${inviteCode} 对应的用户`);
            }
            return user;
        }
        catch (error) {
            this.logger.error(`根据邀请码查找用户时出错: ${error.message}`, error.stack);
            return null;
        }
    }
    async createInvitation(data) {
        this.logger.debug(`创建邀请记录: 邀请人ID ${data.inviterId} -> 被邀请人ID ${data.inviteeId}`);
        try {
            const existingInvitation = await this.invitationRepository.findOne({
                where: { inviteeId: data.inviteeId }
            });
            if (existingInvitation) {
                this.logger.debug(`用户ID ${data.inviteeId} 已有邀请记录，跳过创建`);
                return existingInvitation;
            }
            const invitation = new invitation_entity_1.InvitationEntity();
            invitation.inviterId = data.inviterId;
            invitation.inviteeId = data.inviteeId;
            const result = await this.invitationRepository.save(invitation);
            this.logger.debug(`邀请记录创建成功，ID: ${result.id}`);
            return result;
        }
        catch (error) {
            this.logger.error(`创建邀请记录失败: ${error.message}`, error.stack);
            throw new common_1.HttpException('创建邀请记录失败', common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    async incrementInviteLinkCount(userId) {
        this.logger.debug(`增加用户ID ${userId} 的邀请计数`);
        try {
            const user = await this.userEntity.findOne({
                where: { id: userId },
                select: ['id', 'inviteLinkCount']
            });
            if (!user) {
                this.logger.warn(`未找到用户ID ${userId}，无法更新邀请计数`);
                return;
            }
            const newCount = (user.inviteLinkCount || 0) + 1;
            await this.userEntity.update(userId, { inviteLinkCount: newCount });
            this.logger.debug(`用户ID ${userId} 的邀请计数更新为 ${newCount}`);
        }
        catch (error) {
            this.logger.error(`更新邀请计数时出错: ${error.message}`, error.stack);
        }
    }
    async findById(id) {
...
UserService = UserService_1 = __decorate([
    ...
    __param(1, (0, typeorm_1.InjectRepository)(invitation_entity_1.InvitationEntity)),
