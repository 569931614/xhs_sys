...
let PayService = class PayService {
    constructor(cramiPackageEntity, orderEntity, userBalanceService, globalConfigService, userService, orderService) {
    ...
    async notifyHupi(params) {
        ...
        if (order.status === 1) {
            await this.userBalanceService.addBalanceToOrder(order);
            await this.orderService.handlePaymentSuccess(params['out_trade_no']);
        }
        return 'success';
    }
    ...
    async notifyWeChat(params) {
        ...
        try {
            if (params['event_type'] == 'TRANSACTION.SUCCESS') {
                ...
                if (status === 1) {
                    await this.userBalanceService.addBalanceToOrder(order);
                    await this.orderService.handlePaymentSuccess(resource['out_trade_no']);
                }
                if (result.affected != 1)
                    return 'failed';
            }
    ...
    async payLtzf(userId, orderId, payType = 'wxpay') {
        ...
        if (order.status === 1) {
            await this.userBalanceService.addBalanceToOrder(order);
            await this.orderService.handlePaymentSuccess(params['outOrderNo']);
        }
        return { url_qrcode, url };
    }
...
PayService = __decorate([
    ...
    __param(5, (0, common_2.Inject)((0, common_2.forwardRef)(() => order_service_1.OrderService))),
