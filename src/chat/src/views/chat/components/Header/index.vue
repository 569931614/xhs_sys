...
import { useAppStore, useChatStore, useAppCatStore } from '@/store';
...
const showEmbeddedApp = computed(() => appCatStore.showEmbeddedApp);
const currentEmbeddedApp = computed(() => appCatStore.currentEmbeddedApp);
    ...
    <div class="relative flex items-center justify-center min-w-0 h-full">
      <div class="flex w-full h-full items-center px-4">
        ...
        <div class="flex justify-between items-center h-full w-full">
          <!-- 嵌入式应用模式 -->
          <div v-if="showEmbeddedApp" class="flex-1">
            <!-- 添加应用名称显示 -->
            <div class="inline-flex items-center px-3 py-2 text-lg font-semibold dark:text-gray-200 text-gray-800">
              {{ currentEmbeddedApp?.name || '应用' }}
            </div>
          </div>
          
          <!-- 正常聊天模式 -->
          <Menu
            v-else-if="notSwitchModel"
            as="div"
