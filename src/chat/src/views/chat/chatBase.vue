...
import { useAuthStore, useChatStore, useGlobalStoreWithOut, useAppCatStore } from '@/store';
...
import EmbeddedApp from './components/EmbeddedApp/index.vue';
...
const showEmbeddedApp = computed(() => {
  console.log('appCatStore.showEmbeddedApp', appCatStore.showEmbeddedApp);
  return appCatStore.showEmbeddedApp;
});
...
<template>
  <div class="h-full flex flex-col pb-2 bg-white dark:bg-gray-800">
    ...
    <main class="flex-1 overflow-hidden">
      <div v-if="showEmbeddedApp" class="h-full">
        {{ console.log('chatBase中showEmbeddedApp为true，准备显示嵌入式应用') }}
        {{ console.log('appCatStore.currentEmbeddedApp:', appCatStore.currentEmbeddedApp) }}
        <div class="h-full">
          <EmbeddedApp />
        </div>
      </div>
      ...
      >
        {{ console.log('chatBase中showEmbeddedApp为false，显示常规聊天界面') }}
        <div
          id="image-wrapper"
    ...
    <FooterComponent v-if="!showEmbeddedApp" @pause-request="pauseRequest" />
  </div>
</template>
