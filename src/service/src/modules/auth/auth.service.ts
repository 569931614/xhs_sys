...
export class AuthService {
  ...
  async register(body: any, req: Request) {
    ...
    Logger.debug(
      `Contact: ${contact}, isEmail: ${isEmail}, isPhone: ${isPhone}, inviteCode: ${inviteCode || '无'}`
    );
    ...
    if (inviteCode) {
      ...
      Logger.debug(`用户注册时使用了邀请码: ${inviteCode}`);
    }
    ...
    if (inviteCode) {
      try {
        ...
        if (inviter) {
          Logger.debug(`发现邀请人，ID: ${inviter.id}，用户名: ${inviter.username}`);
          ...
          Logger.debug(`创建邀请记录成功: 邀请人ID ${inviter.id} -> 被邀请人ID ${u.id}`);
          
          // 更新邀请者的邀请计数
          await this.userService.incrementInviteLinkCount(inviter.id);
          Logger.debug(`更新邀请人(${inviter.id})的邀请计数`);
        } else {
          Logger.warn(`未找到使用邀请码 ${inviteCode} 的邀请人`);
        }
      } catch (error) {
        Logger.error(`处理邀请关系时出错: ${error.message}`, error.stack);
        // 不阻止用户注册，仅记录错误
      }
    }
