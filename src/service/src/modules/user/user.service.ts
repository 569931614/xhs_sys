...
export class UserService {
   ...
   * 根据邀请码查找用户
   */
  async findUserByInviteCode(inviteCode: string): Promise<UserEntity> {
   ...
   * 创建邀请记录
   */
  async createInvitation(data: { inviterId: number; inviteeId: number }): Promise<InvitationEntity> {
    ...
    try {
      ...
      throw new HttpException('创建邀请记录失败', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
   ...
   * 增加用户的邀请链接点击计数
   */
  async incrementInviteLinkCount(userId: number): Promise<void> {
