...
export class UploadService implements OnModuleInit {
  ...
  /**
   * 从URL上传文件到Coze
   * @param params 包含url和dir的对象
   * @returns 上传结果，包含cozeFileId
   */
  async uploadFileFromUrl(params: { url: string; dir: string }): Promise<any> {
    try {
      this.logger.log(`从URL上传文件: ${params.url}, 目录: ${params.dir}`);
      ...
      return {
        success: true,
        cozeFileId: result.file_id
      };
    } catch (error) {
      this.logger.error(`从URL上传文件失败: ${error.message}`, error.stack);
      throw error;
    }
  }
